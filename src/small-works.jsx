...
export default function SmallWorks({ orderId, onOrderCreated, onBack }) {
  ...
  useEffect(() => {
    if (workType !== "tezy" || stage !== "sources") return;
    const t = setTimeout(() => {
...
requirements — якщо є рекомендації у файлах, стисло опиши ключові вимоги до структури та оформлення.${tezyHints}`;
      ...
      const model = (isTezy && (files.length > 0 || materialFiles.length > 0)) ? MODEL : MODEL_FAST;
  ...
  const doConfirmTezyPapers = async () => {
    ...
    try {
      ...
      // Gemini-форматування за ДСТУ 8302:2015 (як у великих роботах)
      ...
      try {
        ...
        const fmtPrompt = `СТИЛЬ: ДСТУ 8302:2015. СУВОРО дотримуйся ДСТУ.
Правила:
...
${refLines}`;
        const fmtResult = await callGemini(
          [{ role: "user", content: fmtPrompt }], null,
