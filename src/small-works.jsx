...
Обсяг: ~${totalPages} сторінок. Аналітичний стиль. Без жирного.`,
    ...
    if (workType === "stattia") {
      ...
      articleFrontMatter = isEnglishLang
        ? `\n${info?.needsUDK !== false ? "First line of the document: UDC [appropriate code for the topic]\n" : ""}AUTHOR BLOCK (${authorFormat === "right" ? "right-aligned" : "centered"}, right below the title):\n${authorBlockLines}\n\nAfter the author block — ABSTRACT: start the line with "Abstract." and immediately on the same line write 50-80 words (aim, methods, results). Next line: "Keywords:" followed by 5-7 keywords separated by commas.`
        : `\n${info?.needsUDK !== false ? "Перший рядок документу: УДК [відповідний код для теми]\n" : ""}БЛОК АВТОРА (${authorFormat === "right" ? "вирівняти по правому краю" : "вирівняти по центру"}, одразу під назвою):\n${authorBlockLines}\n\nПісля блоку автора — АНОТАЦІЯ: почни рядок зі слова "Анотація." і одразу на тому ж рядку 50-80 слів анотації (мета, методи, результати). Наступний рядок: "Ключові слова:" і одразу 5-7 ключових слів через кому.`;
    }
