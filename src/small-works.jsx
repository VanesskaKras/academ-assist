...
export default function SmallWorks({ orderId, onOrderCreated, onBack }) {
  ...
  // Тези — матеріал для роботи (текст що аналізується при генерації)
...
needsUDK — чи треба УДК (true/false).` : "";
      ...
      const materialHint = (isTezy && materialText.trim())
...
requirements — якщо є рекомендації у файлах, стисло опиши ключові вимоги до структури та оформлення.${tezyHints}`;
  ...
  const doConfirmTezyPapers = async () => {
    ...
    try {
      ...
      const citations = enriched.map(paperToCitation).filter(Boolean);
            ...
            {workType === "tezy" && (
              <FieldBox label="Матеріал для роботи (необов'язково)">
                <textarea
                  ...
                  placeholder={"Вставте текст, тези, конспект, реферат або будь-який матеріал, на основі якого ШІ має написати тези. ШІ проаналізує його і використає як основу."}
                  style={{ ...TA, minHeight: 130 }}
                />
                {materialText.trim() && (
                  <div style={{ fontSize: 11, color: "#5a8a2a", marginTop: 4 }}>
                    ✓ {materialText.trim().split(/\s+/).length} слів — буде передано ШІ як матеріал для аналізу
                  </div>
                )}
              </FieldBox>
            )}
