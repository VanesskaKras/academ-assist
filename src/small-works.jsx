  ...
  const runSection = async (sec) => {
    ...
    if (sec.id === "intro") {
      ...
      const introStructure = hasMethodIntroReq
        ...
        : `Структура вступу (кожен елемент — окремий абзац, дотримуйся суворо):
Актуальність теми: [95-110 слів про актуальність обраної теми]
Об'єкт дослідження: [одне речення — що саме досліджується]
Предмет дослідження: [одне речення — конкретний аспект об'єкту на якому фокусується робота]
Мета роботи: [одне речення — мета]
Завдання дослідження: [3-4 завдання, кожне з нового рядка, без нумерації]
Структура роботи: [одне речення про кількість розділів та їх зміст]`;
      instruction = `Напиши ВСТУП для реферату на тему "${info?.topic}".
${materialContext}${commentBlock}${introStructure}
...
${materialContext}${commentBlock}Підсумуй основні результати по кожному розділу. Конкретні висновки без загальних фраз.
