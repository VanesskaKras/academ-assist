...
- subsectionsPerChapterOverrides: якщо методичка явно описує різну кількість підрозділів для різних розділів (наприклад, перший розділ має 3 підрозділи, другий — 4) — поверни об'єкт де ключ — номер розділу рядком, значення — кількість підрозділів: {"2": 4}. Включай тільки ті розділи що відрізняються від subsectionsPerChapter. null якщо всі розділи мають однакову кількість або кількість взагалі не вказана
