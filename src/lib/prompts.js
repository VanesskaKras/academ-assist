...
export const STRUCTURE_READING_PROMPT = `Уважно прочитай методичку. Твоє завдання — знайти структуру роботи.

ПОКРОКОВА ІНСТРУКЦІЯ:
1. Знайди текст де описується кожен розділ роботи — зазвичай це блок від слів "Перший розділ" / "Перша частина" / "Розділ 1" до слова "Висновки" або кінця опису структури
2. У цьому діапазоні тексту для КОЖНОГО розділу окремо порахуй скільки підрозділів описано (підрозділи позначаються словами: "перший підрозділ", "другий підрозділ", "третій підрозділ" тощо, або нумерацією 1.1, 1.2, 2.1 тощо)
3. Якщо кількість підрозділів різна у різних розділах — запиши їх окремо
4. Процитуй ДОСЛІВНО ключове речення де вказана структура
...
Правила:
- structureQuote: процитуй ДОСЛІВНО фрагмент де описана структура розділів. null тільки якщо такого тексту немає
...
- subsectionsPerChapter: найпоширеніша к-сть підрозділів на розділ. null якщо не вказано
- subsectionsPerChapterOverrides: якщо розділи мають РІЗНУ к-сть підрозділів — поверни об'єкт де ключ це номер розділу рядком, значення — к-сть підрозділів. Включай тільки ті розділи що відрізняються від subsectionsPerChapter. Приклад: розділ 1 має 2 підрозділи, розділ 2 має 4 — тоді subsectionsPerChapter=2, subsectionsPerChapterOverrides={"2":4}. null якщо всі розділи однакові
- hasChapterConclusions: true якщо є вимога "висновки до розділу", "розділ завершується висновками" або подібне
