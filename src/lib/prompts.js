...
- id "intro"/"conclusions"/"sources" залишати незмінними; для решти використовуй нумерацію зі зразка`;
  ...
  if (structureExampleText) {
    // Зразок є, але БЕЗ підрозділів (суцільний виклад думок, без заголовків) —
    // не підставляємо замість нього вигаданий плоский шаблон за категорією
    // (це теж була б штучна структура, якої в зразку нема). Основна частина
    // лишається одним цілим блоком, як і сам зразок.
    return `Склади структуру звіту з практики за зразком.
Загальний обсяг: ${total} сторінок.

ЗРАЗОК ГОТОВОГО ЗВІТУ (написаний суцільним текстом, без розділів і підрозділів — це нормально, орієнтуйся на цей формат, а НЕ вигадуй заголовки й розділи):
${structureExampleText.slice(0, 20000)}

Поверни ТІЛЬКИ цей JSON без змін (структура фіксована, бо зразок без підрозділів):
{"sections":[
  {"id":"intro","label":"Вступ","pages":${introPages}},
  {"id":"main","label":"Основна частина","pages":${main}},
  {"id":"conclusions","label":"Висновки","pages":${conclPages}},
  {"id":"sources","label":"Список використаних джерел","pages":0}
]}`;
  }
...
export function buildPracticeWritingPrompt(sec, info, methodInfo, clientMaterialsSummary, citInputs, abstractsMap, establishedFacts, deptGuidanceText) {
  ...
  const hint = isIntro
    ...
    : isFreeformMain
    ? `Це єдина основна частина звіту (без поділу на розділи чи підрозділи) — послідовно розкрий і базу практики (структура, чисельність, напрями діяльності), і види робіт та завдання, до яких долучався практикант, і здобутий досвід — суцільним текстом, без підзаголовків усередині.`
    : "";
