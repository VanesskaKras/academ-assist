...
Якщо зауваження каже, що сторінки у внутрітекстових цитатах-посиланнях (напр. "[5, с. 120]") не відповідають реальному обсягу джерела зі списку літератури (сторінка виходить за межі джерела) — це НЕ звичайна правка розділу. Постав ОДИН окремий пункт з "sectionId":"_citation_pages" (не використовуй цей id для жодного іншого випадку), issue і suggestion як завжди, і БЕЗ поля sourcesAction.
...
ВИДІЛЕНИЙ ФРАГМЕНТ: "${annotatedText}"
${context && context !== annotatedText ? `КОНТЕКСТ (абзац): "${context}"` : ""}
${instrLine}

Виділений фрагмент буває двох типів — спершу визнач, який це випадок:
...
Якщо для виправлення бракує даних, яких ти не можеш ні знати, ні перевірити (наприклад, яка насправді сторінка джерела, чи існує факт, який номер правильний) — НЕ вигадуй значення і не залишай текст без змін мовчки. Натомість поверни статус needs_review з коротким поясненням, що саме треба перевірити людині.

Поверни ТІЛЬКИ JSON (без markdown), один із двох варіантів:
{"status":"fixed","original":"точний фрагмент з тексту роботи, який треба замінити (може включати й репліку рецензента, якщо вона частина виділення) — скопіюй дослівно","replacement":"виправлений варіант без репліки рецензента"}
{"status":"needs_review","note":"коротко, чому неможливо виправити автоматично і що саме перевірити людині"}
...
- НЕ повертай весь документ — тільки JSON з полями вище`;
}
...
Якщо для виправлення бракує даних, яких ти не можеш ні знати, ні перевірити (наприклад точний факт, дата, сторінка джерела, номер) — НЕ вигадуй значення і не залишай текст без змін мовчки. Натомість поверни статус needs_review з коротким поясненням, що саме перевірити людині.

Поверни ТІЛЬКИ JSON (без markdown), один із двох варіантів:
{"status":"fixed","original":"точний фрагмент з тексту роботи який треба замінити (від кількох речень до кількох абзаців), без змін","replacement":"виправлений варіант цього фрагменту"}
{"status":"needs_review","note":"коротко, чому неможливо виправити автоматично і що саме перевірити людині"}
