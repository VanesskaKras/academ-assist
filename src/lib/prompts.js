...
{"id_розділу": "повний текст цього розділу", ...}

Правила:
- Кожен id з переліку вище повинен бути ключем
- Якщо розділу немає в тексті — порожній рядок ""
- Включай ВЕСЬ текст розділу без скорочень
- Повертай ТІЛЬКИ валідний JSON`;
}

// ── Розбивка готової частини роботи від клієнта: текст по розділах + джерела, з нормалізацією цитувань у [N] ──
export function buildFileToSectionsWithSourcesPrompt({ sections, documentText }) {
  const sectionsList = sections
    .filter(s => s.type !== "sources")
    .map(s => `- id: "${s.id}", назва: "${s.label}"`)
    .join("\n");

  return `Розбий текст академічної роботи, наданої клієнтом, по розділах відповідно до структури, і для кожного знайденого розділу витягни список джерел, якими він цитується.

СТРУКТУРА РОБОТИ:
${sectionsList}

ПОВНИЙ ТЕКСТ РОБОТИ (включно зі списком використаних джерел, якщо він є в кінці):
${documentText}

Для кожного розділу зі структури вище, який присутній у тексті:
1. Візьми повний текст цього розділу.
2. Знайди джерела, на які посилається текст (у будь-якому форматі — [1], (Автор, рік), виноски тощо), у порядку їх першої появи в тексті розділу. Для кожного знайди повний бібліографічний опис у списку літератури клієнта, якщо він є в документі; якщо опису немає — залиш тільки те, що видно з тексту (автор, рік).
3. Перепиши позначки цитувань у тексті розділу в уніфікований формат [N], де N — порядковий номер джерела в списку з кроку 2 (перше за появою джерело — [1], друге — [2] і т.д.; повторні цитування того самого джерела — той самий номер). Решту тексту НЕ змінюй.

Поверни ТІЛЬКИ JSON об'єкт (без markdown):
{"id_розділу": {"text": "текст розділу з уніфікованими [N]", "sources": ["Джерело 1 — повний бібліографічний опис", "Джерело 2 — повний бібліографічний опис"]}, ...}

Правила:
- Кожен id з переліку вище повинен бути ключем
- Якщо розділу немає в тексті — {"text": "", "sources": []}
- Включай ВЕСЬ текст розділу без скорочень
- Якщо в розділі немає жодного цитування — "sources": []
- Не вигадуй джерела, яких немає в тексті чи списку літератури клієнта
- Повертай ТІЛЬКИ валідний JSON`;
}
