...
export const STRUCTURE_READING_PROMPT = `Уважно прочитай методичку. Твоє завдання — знайти ТІЛЬКИ інформацію про структуру роботи.

ПОКРОКОВА ІНСТРУКЦІЯ:
1. Знайди розділ "Структура роботи", "Структура та зміст роботи", "Будова роботи" або подібний
2. Знайди речення де написано скільки розділів і скільки підрозділів у кожному розділі
3. Процитуй це речення ДОСЛІВНО
4. На основі цитати виведи числа

Поверни ТІЛЬКИ JSON (без markdown):
{
  "structureQuote": "дослівна цитата з методички де вказана структура роботи",
  "totalPages": 30,
  "introPages": null,
...
Правила:
- structureQuote: знайди та процитуй ДОСЛІВНО фрагмент методички де описана структура роботи (к-сть розділів, підрозділів). null тільки якщо такого тексту взагалі немає
- totalPages: загальний обсяг роботи в сторінках (без додатків і списку джерел). Шукай: "обсяг роботи", "загальний обсяг", "не менше X сторінок", "X сторінок основного тексту"
...
- chaptersCount: к-сть основних розділів. Визначай ТІЛЬКИ з основного тексту методички. НЕ рахуй з зразка змісту або з переліку розділів самої методички. null якщо явно не вказано
- subsectionsPerChapter: к-сть підрозділів у кожному розділі. Якщо різна — вкажи найпоширенішу. null якщо явно не вказано
- hasChapterConclusions: true якщо є вимога "висновки до розділу", "розділ завершується висновками", "кожен розділ має містити висновки" або подібне. false якщо такої вимоги немає
- chapterTypes: масив довжиною chaptersCount. "theory" — теоретичний огляд літератури, "analysis" — аналітичний або практичний, "recommendations" — рекомендаційний або розробка або впровадження`;
...
${s.structureQuote ? `Цитата з методички: "${s.structureQuote}"` : ''}
