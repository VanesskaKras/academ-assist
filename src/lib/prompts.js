...
Поверни ТІЛЬКИ JSON-масив (без markdown), РІВНО по одному об'єкту на кожне завдання зі списку вище, у тому самому порядку — "index" відповідає номеру завдання (0, 1, 2...). Поле "original" НЕ потрібне — програма сама точно знає, який фрагмент виділено, і замінить саме його:
[{"index":0,"status":"fixed","replacement":"виправлений варіант ВСЬОГО виділеного фрагмента (включно з тим, що там був репліку рецензента — тепер без неї)"}]
Для needs_review: {"index":1,"status":"needs_review","note":"коротко, чому неможливо виправити автоматично"}

Правила:
- "replacement" — це ПОВНА заміна ВСЬОГО виділеного фрагмента "ВИДІЛЕНИЙ ФРАГМЕНТ" відповідного завдання, зберігаючи стиль і мову оригіналу; репліки рецензента в ньому бути не повинно
- Якщо виділений фрагмент підлягає повному видаленню (наприклад це лише репліка рецензента без окремого дефекту поруч) — "replacement" пустий рядок
- Завдання незалежні одне від одного
- Якщо "replacement" містить символ лапок " — обов'язково екрануй його зворотним слешем всередині JSON-рядка, інакше весь масив не розпарситься
